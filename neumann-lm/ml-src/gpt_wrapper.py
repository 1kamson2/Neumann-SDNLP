...
class GPTWrapper:
    def __init__(self):
        self.__api_key = open("API_KEY", "r").read().rstrip('\n')
        self.__client = openai.OpenAI(api_key=self.__api_key)

    def create_prompt(self, helper_content: str,
                           user_content: str):  
        ...
        return {"HELPER" : prompt_helper("system", helper_content),
...
def gpt_save_to_file(result): 
    try:
        with open("GPT_OUT", "w") as _in:
            _in.write(result.content)
    except IOError as e:
        print(f"[FAILED WHILE SAVING RESULT TO FILE]: {e}")
    finally:
        print("Cleaning up...")
            
def gpt_client_run(user_question):
    gpt_client = gpt_client_builder()
    prompts = gpt_client.create_prompt("You are a helpful assistant",
                                            user_question)
